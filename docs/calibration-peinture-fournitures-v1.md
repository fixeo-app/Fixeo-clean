# Peinture des murs avec fournitures — proposition à valider

État au 21 septembre 2026 : **préparation testée, tarif non approuvé et non activé**.
...
- Même proposition dans les 20 villes du catalogue ; cela ne garantit ni stock fournisseur ni disponibilité partenaire.
...
| Surface | Quantité calculée avec pertes | Exemple d’achat au conditionnement repère | Coût repère avant transport | Forfait proposé |
|---|---:|---:|---:|---:|
...
Les vingt nouvelles entrées sont `approved: false` avec `PENDING_OWNER_PRICE_VALIDATION`. Les indicateurs d’activation et de préparation production sont faux. Le moteur permet de tester les montants proposés ; le serveur refuse de persister une offre ou de signer un prix réservable tant que le catalogue n’est pas approuvé. Les tests simulent l’approbation en mémoire uniquement. Le champ historique `human_decision: APPROVED` décrit l’ancien service et ne vaut pas validation de ce nouveau tarif.

Cette branche ne doit pas être fusionnée en production avant validation du tarif et du périmètre. La validation suivante doit approuver les vingt entrées et mettre à jour les indicateurs, la provenance et le présent document. Les références du script client portent déjà une nouvelle version pour renouveler le cache lors du futur déploiement.
...
Résultat du 21 septembre 2026 : **84 tests réussis, aucun échec** dans la suite complète de l’estimateur. Les deux rapports JSON protégés sont inchangés, tout comme les tarifs approuvés des autres prestations.
...
Message de commit : `feat(pricing): prepare supplied wall painting tariff for approval`
