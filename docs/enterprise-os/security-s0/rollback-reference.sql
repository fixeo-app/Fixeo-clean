-- BLOCKED REFERENCE ONLY. NOT AUTHORIZED. NO AUTOMATIC ROLLBACK.
...
ORDER BY check_name
 ...
 IF failed IS NOT NULL THEN RAISE EXCEPTION 'S0 rollback_preflight failed: %', failed USING ERRCODE='P0001'; END IF;
END $s0_rollback_preflight$;
GRANT EXECUTE ON FUNCTION public.auto_dispatch_service_request_v1() TO PUBLIC;
GRANT EXECUTE ON FUNCTION public.auto_dispatch_service_request_v1() TO anon;
GRANT EXECUTE ON FUNCTION public.auto_dispatch_service_request_v1() TO authenticated;
GRANT EXECUTE ON FUNCTION public.dispatch_notification_worker_peek_v1(text) TO PUBLIC;
GRANT EXECUTE ON FUNCTION public.dispatch_notification_worker_peek_v1(text) TO anon;
GRANT EXECUTE ON FUNCTION public.dispatch_notification_worker_peek_v1(text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.dispatch_request_v1_backup_20260821(uuid) TO anon;
GRANT EXECUTE ON FUNCTION public.dispatch_request_v1_backup_20260821(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.enqueue_dispatch_notification_from_mission_v1() TO PUBLIC;
GRANT EXECUTE ON FUNCTION public.enqueue_dispatch_notification_from_mission_v1() TO anon;
GRANT EXECUTE ON FUNCTION public.enqueue_dispatch_notification_from_mission_v1() TO authenticated;
GRANT EXECUTE ON FUNCTION public.trigger_dispatch_v2_on_service_request() TO PUBLIC;
GRANT EXECUTE ON FUNCTION public.trigger_dispatch_v2_on_service_request() TO anon;
GRANT EXECUTE ON FUNCTION public.trigger_dispatch_v2_on_service_request() TO authenticated;
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON TABLE public.artisan_profiles TO anon;
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON TABLE public.artisan_profiles TO authenticated;
GRANT MAINTAIN ON TABLE public.artisans TO anon;
GRANT MAINTAIN ON TABLE public.artisans TO authenticated;
GRANT MAINTAIN ON TABLE public.enterprise_accounts TO authenticated;
GRANT MAINTAIN ON TABLE public.enterprise_audit_events TO authenticated;
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON TABLE public.missions TO anon;
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON TABLE public.missions TO authenticated;
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON TABLE public.notifications TO anon;
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON TABLE public.notifications TO authenticated;
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON TABLE public.quotes TO anon;
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON TABLE public.quotes TO authenticated;
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON TABLE public.service_requests TO anon;
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON TABLE public.service_requests TO authenticated;
DO $s0_rollback_postflight$
DECLARE failed text;
BEGIN
 SELECT string_agg(check_name, ', ' ORDER BY check_name) INTO failed FROM (
WITH captured AS (
...
SELECT jsonb_build_object(
 ...
 md5(c.contract->>e.key) AS actual_md5,
 md5(c.contract->>e.key) = e.value AS pass
 FROM captured c CROSS JOIN jsonb_each_text('{"policies":"57741de3e2d77d408bfafeb777834e05","triggers":"54e1d0cf96c413639e7de248e332d183","column_acl":"9e5791ff271403b7d7db9c7c741a42a7","constraints":"0f5f8e80395244eea2289b25bb7660be","schema_access":"0891ab6a658cf605bf691aacb854aaa5","table_effective":"a4dc3dd5b25ad264715fa2bd6b025024","table_inventory":"21ef0c8492b7f51c32c23934248e7b1e","function_inventory":"ab5347eb59627f227bd8f0086c7da7a1","unprivileged_roles":"05b193a66e730ecc334ccfbc3093cb0d","all_function_definitions":"2e90810debf53c495e80bc72d14657c2","browser_role_memberships":"d751713988987e9331980363e24189ce"}'::jsonb) e
)
SELECT * FROM checks
UNION ALL SELECT 'ALL_CHECKS', NULL, NULL, bool_and(pass) FROM checks
ORDER BY check_name
 ) checks WHERE pass IS DISTINCT FROM true;
 IF failed IS NOT NULL THEN RAISE EXCEPTION 'S0 rollback_postflight failed: %', failed USING ERRCODE='P0001'; END IF;
END $s0_rollback_postflight$;
COMMIT;
