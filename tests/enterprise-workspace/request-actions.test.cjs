...
test('Block A create uses hybrid enterprise request RPC',async()=>{
  const c=client();
  ...
  assert.deepEqual(c.calls,[{name:'create_enterprise_request_hybrid',args:{
...
test('B3 source has no direct table write or service role authority',()=>{
  ...
  assert.match(code,/create_enterprise_request_hybrid/);
});
