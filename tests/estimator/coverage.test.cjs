...
test('new métiers offer a quote route and never a fabricated tariff',()=>{for(const metier of ['demenagement','maconnerie']){assert.ok(resolver.VALID_METIERS.includes(metier));const s=o.startEstimator({metier_hint:metier}).session;const choices=o.getNextEstimatorStep(s).step.candidate_services;assert.equal(choices.length,1);assert.match(choices[0].service_code,/^devis\./);assert.equal(o.selectService(s,choices[0].service_code).session.state,'QUOTE_REQUIRED');}});
