...
const pilot=require('../../data/pricing/engine/plumbing-pilot-v1');
...
async function setup(){const db=new PGlite();
 ...
 await db.exec("INSERT INTO artisans(city,service_category,availability,review_count,rating,updated_at) VALUES ('Rabat','Carrelage','available',500,5,now()),('Rabat','Plomberie','available',0,0,now());");
...
async function mission(db,code='plomberie.diagnostic'){
 ...
 const s=o.evaluateEstimator(o.startEstimator({service_hint:code,city_slug:'rabat',known_inputs:pilot.services[code].inputs}).session).session;
...
async function propose(db,m,code,paid=22000,id=randomUUID(),inputs=pilot.services[code].inputs,visit=true){
 await asArtisan(db);return (await db.query('SELECT plumbing_repair_artisan_v1($1,$2,$3,$4,$5,$6) AS r',[m.id,code,JSON.stringify(inputs),paid,id,visit])).rows[0].r;
}
async function decide(db,m,p,action='accept',hash='a'.repeat(64),paid=p.diagnostic_paid_minor){
 await db.exec('RESET ROLE; SET ROLE service_role');return (await db.query('SELECT plumbing_repair_guest_v1($1,$2,$3,$4,$5) AS r',[m.ref,hash,action,p.id,paid])).rows[0].r;
}
test('Approved plumbing: actual offer → atomic reservation → plumbing dispatch → single commission',async()=>{
 const db=await setup();try{
  for(const [code,s] of Object.entries(pilot.services)){
   await db.exec('RESET ROLE');
   assert.deepEqual((await db.query('SELECT fixeo_private.plumbing_tariff_v1($1) AS t',[code])).rows[0].t,s);
   const m=await mission(db,code);
   assert.equal((await db.query('SELECT service_category FROM artisans WHERE id=$1',[m.artisan_profile_id])).rows[0].service_category,'Plomberie');
...
test('All five same-visit repairs, with or without diagnostic cash already received, keep one mission and immutable offers',async()=>{
 const db=await setup();try{
  for(const [code,s] of Object.entries(pilot.services).filter(([c])=>c!=='plomberie.diagnostic'))for(const paid of [0,22000]){
   const m=await mission(db),id=randomUUID();const proposed=await propose(db,m,code,paid,id);
   assert.equal(proposed.current_total_minor,22000);assert.equal(proposed.proposal.status,'PENDING');
   assert.deepEqual(await propose(db,m,code,paid,id),proposed);
   await assert.rejects(propose(db,m,code,paid===0?22000:0,id),/Idempotency conflict/);
...
test('Consent, ownership, scope, latest proposal, expiry, lifecycle and privilege boundaries fail closed',async()=>{
 const db=await setup();try{
  const m=await mission(db),code='plomberie.fuite_simple';
  ...
  await assert.rejects(db.query('SELECT plumbing_repair_artisan_v1($1)',[m.id]),/Mission not available/);
  await asArtisan(db);
  await assert.rejects(db.query('SELECT plumbing_repair_guest_v1($1,$2)',[m.ref,'a'.repeat(64)]),/permission denied/);
  await assert.rejects(db.query('SELECT * FROM fixeo_private.plumbing_proposals_v1'),/permission denied/);
  ...
  const p3=(await propose(db,m,'plomberie.chasse_eau')).proposal;
  ...
  await db.exec('RESET ROLE');
  await assert.rejects(db.query('DELETE FROM fixeo_private.plumbing_decisions_v1 WHERE proposal_id=$1',[p.id]),/immutable/);
  assert.equal((await db.query('SELECT count(*) AS n FROM fixeo_private.plumbing_decisions_v1 WHERE accepted')).rows[0].n,0);
  ...
  await db.exec('RESET ROLE');
  const src=(await db.query('SELECT * FROM fixeo_private.plumbing_proposals_v1 WHERE id=$1',[pid])).rows[0];
  ...
  const expiredId=randomUUID();await db.query('INSERT INTO fixeo_private.plumbing_proposals_v1(id,mission_id,request_id,artisan_id,original_offer_id,repair_offer_id,diagnostic_paid_minor,same_visit) VALUES($1,$2,$3,$4,$5,$6,0,true)',[expiredId,src.mission_id,src.request_id,src.artisan_id,src.original_offer_id,oldId]);
