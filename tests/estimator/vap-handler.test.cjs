...
test('Real estimator handler signs persisted VAP and forwards its exact total to atomic confirmation',async()=>{
 ...
 registry.entries.push({approved:true,service_code:'plomberie.fuite_simple',city_slug:'rabat',outcome_type:'PRICE_READY',inputs:{plumbing_scope:'LOCAL_ACCESSIBLE'},vap_minor:30001,materials_minor:0,catalogue_version:'test-only'});
 ...
 try{
  const session=orchestrator.startEstimator({service_hint:'plomberie.fuite_simple',city_slug:'rabat',known_inputs:{plumbing_scope:'LOCAL_ACCESSIBLE'}}).session;
  session.entry_context.city_slug='rabat';
  const view=normalizeSessionView(session,process.env.FIXEO_ESTIMATOR_SECRET);
  const r=await api({action:'evaluate',session_token:view.session_token});assert.equal(r.status,200,JSON.stringify(r));assert.equal(r.body.outcome.price.amount_mad,360.01);assert.equal(r.body.outcome.financial_breakdown.commissionMinor,6000);assert.equal(calls.length,1);
  const verified=await api({action:'verify_pricing_context',pricing_context_token:r.body.pricing_context_token});assert.equal(verified.body.amount_mad,360.01);
  ...
  assert.equal(confirmed.status,409);assert.match(calls[1].url,/confirm_estimator_request_vap_v1$/);assert.equal(calls[1].body.p_amount_mad,360.01);assert.equal(calls[1].body.p_offer_id,calls[0].body.id);
 }finally{registry.entries.pop();global.fetch=oldFetch;keys.forEach((k,i)=>{if(old[i]===undefined)delete process.env[k];else process.env[k]=old[i]});}
});
