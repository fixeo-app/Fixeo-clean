...
test('Moving confirmation, dispatch and settlement retain the team total',async()=>{
 ...
 await db.exec("INSERT INTO artisans(city,service_category,availability,review_count,rating,updated_at) VALUES ('Rabat','Carrelage','available',500,5,now()),('Rabat','Déménagement','available',0,0,now());");
 const id=(await db.query(`INSERT INTO fixeo_pricing_offers_v1(offer_key,pricing_version,currency,service_code,catalogue_version,city,scope,vap_minor,materials_minor,commission_minor,client_total_minor,expires_at) VALUES(gen_random_uuid(),'vap-bp33-v1','MAD','demenagement.manutention_2h','test','rabat','{"context_id":"fxctx-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","session_id":"session","outcome_type":"PRICE_READY"}',60000,0,9000,69000,now()+interval '15 minutes') RETURNING id`)).rows[0].id;
 const args=['fxctx-'+'a'.repeat(32),'PRICE_READY','demenagement.manutention_2h','session','690','rabat','0612345678','Test only','FX-TEST','a'.repeat(64),id];
 const sql='SELECT confirm_estimator_request_vap_v1('+args.map((_,i)=>'$'+(i+1)).join(',')+') AS r';
 await db.exec('SET ROLE service_role');
 let result=(await db.query(sql,args)).rows[0].r;assert.equal(result.ok,true);const request=result.request_id;assert.equal(result.service_category,'demenagement');
 assert.equal((await db.query(sql,args)).rows[0].r.replayed,true);
 assert.match((await db.query('SELECT description FROM service_requests WHERE id=$1',[request])).rows[0].description,/2 intervenants pendant 2 heures/);
 const bad=[...args];bad[4]='360';assert.equal((await db.query(sql,bad)).rows[0].r.reason,'offer_mismatch');
 const preview=(await db.query('SELECT * FROM dispatch_preview_v21($1)',[request])).rows;assert.equal(preview.length,1);assert.equal(preview[0].artisan_category,'Déménagement');
 ...
 assert.equal((await db.query('SELECT service_category FROM artisans WHERE id=$1',[mission.artisan_profile_id])).rows[0].service_category,'Déménagement');
 assert.equal(mission.agreed_price,'690.0000000000000000');assert.equal(Number(mission.commission_amount),90);
 await db.query('UPDATE missions SET final_price=690 WHERE id=$1',[mission.id]);
 await db.query('UPDATE missions SET final_price=690 WHERE id=$1',[mission.id]);
 ...
 assert.equal(Number((await db.query('SELECT commission_amount FROM missions WHERE id=$1',[mission.id])).rows[0].commission_amount),90);
