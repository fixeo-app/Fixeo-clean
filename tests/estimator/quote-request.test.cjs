...
test('quote opening does not submit; invalid phone is rejected',async()=>{const s=setup(()=>{throw Error('unexpected');});assert.equal(s.calls.length,0);s.phone.value='123';s.submit();await settle();assert.equal(s.calls.length,0);assert.match(s.w.document.body.textContent,/numéro marocain/);});
test('quote submits once, no price/artisan identity; success requires stored request id',async()=>{
 let finish;const s=setup(()=>new Promise(r=>finish=r));s.phone.value='0612345678';s.submit();s.submit();await settle();assert.equal(s.calls.length,1);
 ...
 finish({ok:true,json:async()=>({ok:true,id:'abc',ref:'FX-ABC'})});await settle();assert.match(s.w.document.body.textContent,/enregistrée/);assert.match(s.w.document.body.textContent,/Aucune intervention n’est réservée/);
});
test('lost response retries same payload and same idempotency key; back preserves fields',async()=>{
 const s=setup(n=>n===1?Promise.reject(Error('lost')):Promise.resolve({ok:true,json:async()=>({ok:true,id:'abc',replayed:true})}));s.phone.value='0612345678';s.submit();await settle();assert.equal(s.calls.length,1);assert.match(s.w.document.body.textContent,/pas été reçue/);s.submit();await settle();assert.equal(s.calls.length,2);assert.deepEqual(s.calls[0].payload,s.calls[1].payload);
});
test('uncertain prior submit blocks changed payload instead of creating another request',async()=>{
 const s=setup(()=>Promise.reject(Error('lost')));s.phone.value='0612345678';s.submit();await settle();s.w.document.getElementById('rafi-quote-description').value='Autre projet';s.submit();await settle();assert.equal(s.calls.length,1);assert.match(s.w.document.body.textContent,/mêmes informations/);
});
test('same payload after modal recreation reuses key from session storage',async()=>{
 const s=setup(()=>Promise.reject(Error('lost')));s.phone.value='0612345678';s.submit();await settle();const old=s.calls[0].payload.idempotency_key;s.m._quoteAttempt=null;s.m._renderQuoteRequest({service_code:'devis.jardinage',service_label:'Entretien jardin'});s.submit();await settle();assert.equal(s.calls.length,2);assert.equal(s.calls[1].payload.idempotency_key,old);
});
