...
async function attachOffer(session,payload,{entries=catalogue.entries,fetchImpl=fetch,env=process.env}={}){
 const tariff=selectTariff(session,entries);if(!tariff){if(/^(plomberie|electricite|climatisation|jardinage|carrelage|maconnerie|demenagement|peinture)\./.test(session.service_code||''))throw Error('Service scope or city not eligible for VAP');return null;}
