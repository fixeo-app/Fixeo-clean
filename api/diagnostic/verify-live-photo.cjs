'use strict';
// Manual live photo E2E only. Never invoked by the Vercel build.
// Requires explicit confirmation and Production credentials in the process environment.
if (!process.argv.includes('--confirm-live-photo')) {
  console.error('DIAGNOSTIC_LIVE_E2E_REQUIRES_EXPLICIT_CONFIRMATION');
  process.exit(1);
}
// Uses disposable Diagnostic dossiers only, never bookings.
...
async function verify() {
  ...
  // configured public flag; this script is never routed as an HTTP endpoint.
  ...
  try {
    ...
    console.log('DIAGNOSTIC_LIVE_E2E_PASS: private upload, WebP metadata removal, real OpenAI analysis, owner isolation, authenticated cleanup.');
  } finally {
    if (mediaId && session) {
...
const timer = setTimeout(() => { console.error('DIAGNOSTIC_LIVE_E2E_FAILED: TIMEOUT'); process.exit(1); }, 120000);
verify().then(() => clearTimeout(timer)).catch(error => {
  clearTimeout(timer);
  const code = /^[A-Z_]{3,80}$/.test(error?.message || '') ? error.message : 'CHECK_FAILED';
  console.error('DIAGNOSTIC_LIVE_E2E_FAILED: ' + code);
  process.exitCode = 1;
});
