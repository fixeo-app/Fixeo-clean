...
const schema={
  ...
  properties:{
    ...
    action_proposals:{type:'array',maxItems:3,items:{type:'object',additionalProperties:false,required:['type','title','summary','target','params','impact'],properties:{type:{type:'string',enum:['create_governed_request','propose_internal_assignment','propose_hybrid_dispatch','decide_approval','upsert_control_tower_escalation','prepare_maintenance_action']},title:{type:'string'},summary:{type:'string'},target:{type:'object',additionalProperties:false,properties:{enterprise_id:{type:['string','null']},site_id:{type:['string','null']},request_id:{type:['string','null']},case_id:{type:['string','null']},plan_id:{type:['string','null']},member_id:{type:['string','null']}}},params:{type:'object',additionalProperties:false,properties:{service_category:{type:['string','null']},description:{type:['string','null']},urgency:{type:['string','null']},requested_amount:{type:['number','null']},decision:{type:['string','null']},note:{type:['string','null']},status:{type:['string','null']}}},impact:{type:'string'}}}}
  }
};
