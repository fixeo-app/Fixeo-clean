...
--
--   Why enterprise_accounts, not enterprise_members?
--   Locking the parent enterprise row serializes ALL concurrent
--   owner-sensitive mutations for the same enterprise. Any two
--   concurrent transactions that want to touch owners of the same
--   enterprise will compete for this single lock. Only one proceeds
--   at a time. The COUNT check then executes against a stable
--   snapshot — no other transaction can change the owner count
--   for this enterprise until the lock holder commits or rolls back.
--
--   Locking only the target enterprise_members row (FOR UPDATE on
--   the row being modified) would not prevent two concurrent
--   transactions from simultaneously modifying two DIFFERENT owner
--   rows of the same enterprise. The parent-lock pattern closes
--   that gap.
--
--   Deadlock prevention: we always lock enterprise_accounts first,
--   then read enterprise_members. This is a single-lock acquisition
--   — no secondary lock ordering is needed.
--
-- TRIGGER TIMING: BEFORE UPDATE OR DELETE, FOR EACH ROW
--   BEFORE: fires before the row change is applied; RAISE inside
--   a BEFORE trigger cancels the statement and rolls back to the
--   implicit savepoint. An AFTER trigger would fire after the row
--   is already modified in the transaction snapshot, making the
--   count check incorrect.
--   FOR EACH ROW: required to access OLD.role, OLD.status.
-- ════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF OLD.role = 'owner' AND OLD.status = 'active' THEN
    ...
    THEN
      -- Acquire exclusive lock on the parent enterprise row.
      -- This serializes all concurrent owner-sensitive mutations
      -- for this enterprise. READ COMMITTED + FOR UPDATE is
      -- race-free for this invariant.
      PERFORM id
...
DO $$ BEGIN RAISE NOTICE '7c15a1 — owner-safety trigger installed'; END $$;
...
--      (serialized by FOR UPDATE lock on enterprise_accounts)
--   D. Caller must be active owner or admin of the enterprise
--   E. Target must be an active or suspended member of the enterprise
--
-- VALID ROLE TRANSITIONS (any non-owner role → any non-owner role)
--   owner  → admin/ops_mgr/site_mgr/reporter/viewer : blocked (Invariant C if last)
--          : owner changing own role away from owner: also blocked by Invariant C
--   admin  → ops_mgr/site_mgr/reporter/viewer : allowed (by owner)
--   *      → owner : BLOCKED always (Invariant A)
-- ════════════════════════════════════════════════════════════
...
DO $$ BEGIN RAISE NOTICE '7c15a1 — update_enterprise_member_role() created'; END $$;
...
--
-- BLOCKED TRANSITIONS
--   removed   → *          (terminal state — no reactivation)
--   *         → active     by admin if target is owner (Invariant B)
--   last active owner → suspended or removed (Invariant C)
--
-- NOTE: 'invited' status is not managed here (deferred invitation flow).
-- ════════════════════════════════════════════════════════════
