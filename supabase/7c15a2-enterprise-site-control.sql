...
--
-- RETURN SHAPE
--   Success: {"ok": true, "site_id": "<uuid>", "enterprise_id": "<uuid>"}
...
DO $$ BEGIN RAISE NOTICE '7c15a2 SECTION 1 — update_enterprise_site() RPC created'; END $$;
...
--
-- RETURN SHAPE
--   Success: {"ok": true, "site_id": "<uuid>", "new_status": "<text>"}
--   No-op:   {"ok": true, "reason": "no_change"}
--   Failure: {"ok": false, "reason": "<code>"}
-- ════════════════════════════════════════════════════════════
...
DO $$ BEGIN RAISE NOTICE '7c15a2 SECTION 2 — set_enterprise_site_status() RPC created'; END $$;
...
-- Remove all UPDATE privileges (table-level and column-level)
-- from authenticated on enterprise_sites.
...
-- No column-level UPDATE grant for authenticated anymore.
-- All site mutations go through RPCs only.
