-- Versioned path only; no historical amounts are rewritten.
SET LOCAL lock_timeout='3s';
SET LOCAL statement_timeout='30s';
ALTER TABLE public.estimator_context_redemptions ALTER COLUMN amount_mad TYPE numeric(12,2);
ALTER TABLE public.service_requests ADD COLUMN pricing_offer_id uuid UNIQUE REFERENCES public.fixeo_pricing_offers_v1(id);
ALTER TABLE public.missions ADD COLUMN pricing_offer_id uuid REFERENCES public.fixeo_pricing_offers_v1(id);
CREATE INDEX missions_pricing_offer_idx ON public.missions(pricing_offer_id) WHERE pricing_offer_id IS NOT NULL;
CREATE SCHEMA IF NOT EXISTS fixeo_private;
REVOKE ALL ON SCHEMA fixeo_private FROM PUBLIC;
GRANT USAGE ON SCHEMA fixeo_private TO authenticated,service_role;
-- Read-only predicate: private schema, no offer amounts or personal data exposed.
CREATE FUNCTION fixeo_private.is_vap_request(request_key text) RETURNS boolean
LANGUAGE sql STABLE SECURITY DEFINER SET search_path='' AS $$
 SELECT EXISTS(SELECT 1 FROM public.service_requests WHERE id::text=request_key AND pricing_offer_id IS NOT NULL)
$$;
REVOKE ALL ON FUNCTION fixeo_private.is_vap_request(text) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION fixeo_private.is_vap_request(text) TO authenticated,service_role;
CREATE FUNCTION fixeo_private.guard_vap_write() RETURNS trigger
...
BEGIN
 ...
 RETURN NEW;
END $$;
REVOKE ALL ON FUNCTION fixeo_private.guard_vap_write() FROM PUBLIC,anon,authenticated;
CREATE TRIGGER a_vap_request_guard BEFORE INSERT OR UPDATE ON public.service_requests FOR EACH ROW EXECUTE FUNCTION fixeo_private.guard_vap_write();
CREATE TRIGGER a_vap_mission_guard BEFORE INSERT OR UPDATE ON public.missions FOR EACH ROW EXECUTE FUNCTION fixeo_private.guard_vap_write();
-- Runs after the historical commission trigger. Reads only the bound immutable offer.
CREATE FUNCTION fixeo_private.mission_vap_amounts() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $$
DECLARE offer public.fixeo_pricing_offers_v1%ROWTYPE; bound uuid;
BEGIN
 IF TG_OP='UPDATE' THEN
  ...
  IF NEW.request_id IS DISTINCT FROM OLD.request_id THEN RAISE EXCEPTION 'Request binding is immutable'; END IF;
 ...
 END IF;
 SELECT * INTO STRICT offer FROM public.fixeo_pricing_offers_v1 WHERE id=bound;
 IF NEW.final_price IS NOT NULL AND NEW.final_price IS DISTINCT FROM offer.client_total_minor::numeric/100 THEN
  RAISE EXCEPTION 'New scope requires a new accepted offer';
 END IF;
...
CREATE TRIGGER z_vap_mission_amounts BEFORE INSERT OR UPDATE ON public.missions FOR EACH ROW EXECUTE FUNCTION fixeo_private.mission_vap_amounts();
...
;
REVOKE ALL ON FUNCTION public.confirm_estimator_request_vap_v1(text,text,text,text,numeric,text,text,text,text,text,uuid) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.confirm_estimator_request_vap_v1(text,text,text,text,numeric,text,text,text,text,text,uuid) TO service_role;
