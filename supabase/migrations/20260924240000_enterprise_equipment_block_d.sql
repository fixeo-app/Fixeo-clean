...
-- Automatically carry plan→equipment linkage onto preventive requests generated by Block C.
CREATE OR REPLACE FUNCTION fixeo_private._fixeo_link_generated_maintenance_equipment_v1()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path=''
...
BEGIN
  ...
  THEN
    INSERT INTO public.enterprise_equipment_request_links(
      enterprise_id,equipment_id,service_request_id,created_by,created_at
    )
    SELECT
      ml.enterprise_id,
      ml.equipment_id,
      ...
      p.created_by,
      now()
    FROM public.enterprise_equipment_maintenance_links ml
    JOIN public.enterprise_maintenance_plans p ON p.id=ml.plan_id
    WHERE ml.plan_id=NEW.plan_id
      AND ml.enterprise_id=NEW.enterprise_id
...
CREATE TRIGGER trg_enterprise_maintenance_run_equipment_link
AFTER INSERT OR UPDATE OF status,service_request_id
ON public.enterprise_maintenance_runs
FOR EACH ROW
EXECUTE FUNCTION fixeo_private._fixeo_link_generated_maintenance_equipment_v1();

REVOKE ALL ON FUNCTION fixeo_private._fixeo_link_generated_maintenance_equipment_v1()
