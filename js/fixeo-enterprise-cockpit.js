...
nav.addEventListener('click',function(e){go(e.target.closest('[data-cockpit-target]'))});d.addEventListener('click',function(e){var b=e.target.closest('.fxew-j2v2-home [data-cockpit-target],.fxew-nav-rafi[data-cockpit-target]');if(b)go(b)});var m=(window.location.hash||'').match(/^#view=(enterprise-[a-z0-9-]+)$/);if(m&&ids.indexOf(m[1])>-1)route(m[1]);
}
